            ...
            if (count === list.length && play === true) {            // 结束
                ...
                if ((tableDate.deadline || tableDate.end) ? (tableDate.deadline < Date.now() || tableDate.end < Date.now()) : false) {  // 没有该属性默认没有结课
                    ...
                    (async function () {
                        ...
                        const element_study_status = document.querySelector("#app > div.viewContainer > div > section > main > div:nth-child(1) > div:nth-child(2) > div > div > div.prepare_info_box.flexbox > div.chart_box > div");
                        const progress_text = element_study_status.textContent.trim()
                        console.log('文本内容：' + progress_text)
                        // 匹配学习进度
                        const progressMatch = progress_text.match(/学习进度：(\d+)\/(\d+)/);
                        if (progressMatch) {
                            const finishedNum = parseInt(progressMatch[1], 10);
                            const totalNum = parseInt(progressMatch[2], 10);
                            console.log(`学习进度：${finishedNum}/${totalNum}`);

                            // 判断是否已经完成
                            if (finishedNum === totalNum || finishedNum > totalNum) {
                                console.log('学习进度已完成');
                            } else {
                                console.log('学习进度未完成');
                                if (classType == '课件PPT') {  // 课件为ppt
                                    ...
                                    for (let i = 0; i < allPPT.length; i++) {
                                        await new Promise(function (resolve) {
                                            // 检查未读标志
                                            var flag = allPPT[i].querySelector('.flag');
                                            console.log(flag)
                                            if (flag && flag.classList.contains('noRead')) {
                                                // 处理未读卡片的逻辑
                                                console.log('卡片 ' + (i + 1) + ' 未读, 处理');
                            ...
                            // 如果没有匹配到学习进度，等待1秒后重新匹配
                            console.log('未匹配到学习进度，等待1秒后重新匹配');
                            setTimeout(studentCards, 1000); // 1秒后再次查找
                        }
                        count++;
                        $.userInfo.setProgress(baseUrl, count);
                        play = true;
